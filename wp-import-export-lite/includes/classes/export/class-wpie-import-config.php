<?php
/**
 * WPIE Import Config Generator Class
 *
 * Generates configuration JSON for export packages, mapping exported fields
 * to import-ready configuration schema.
 *
 * @package    WPIE
 ...
 *
 * Generates import configuration mapping file for export bundles.
 */
class WPIE_Import_Config {
	...
	/**
	 * Generate config.json for the export package.
	 *
	...
	public static function generate( $options = array() ) {
		...
		self::$export_type = isset( $options['wpie_export_type'] ) ? (string) $options['wpie_export_type'] : 'post';
	...
	private static function set_extra_fields() {
		...
		if ( 'product_attributes' === self::$export_type ) {
			self::set_attribute_fields();
		} elseif ( 'taxonomies' === self::$export_type ) {
			self::set_taxonomy_fields();
		} elseif ( 'product' === self::$export_type ) {
			self::set_product_fields();
		} elseif ( 'shop_order' === self::$export_type ) {
			self::set_shop_order_fields();
		} elseif ( 'comments' === self::$export_type || 'product_reviews' === self::$export_type ) {
			self::set_comment_fields();
		} elseif ( 'shop_customer' === self::$export_type ) {
			self::set_customer_fields();
		}
	}

	/**
	 * Set product attribute fields.
	 *
	 * @return void
	 */
	...
	/**
	 * Set WooCommerce product variation fields.
	 *
	 * @return void
	 */
	...
	/**
	 * Set WooCommerce shop order mapping fields.
	 *
	 * @return void
	 */
	private static function set_shop_order_fields() {
		...
		$item_count = isset( self::$options['wpie_order_item_count'] ) ? absint( self::$options['wpie_order_item_count'] ) : 0;
	...
	/**
	 * Get sanitized export filename.
	 *
	 * @return string
	 */
	private static function get_filename() {

		$type = isset( self::$options['wpie_export_file_type'] ) && ! empty( self::$options['wpie_export_file_type'] ) ? (string) self::$options['wpie_export_file_type'] : 'csv';

		$fileName = isset( self::$options['fileName'] ) ? (string) self::$options['fileName'] : '';

		if ( 'csv' !== $type && '' !== $fileName ) {
	...
	/**
	 * Write config.json to export folder.
	 *
	 * @return void
	 */
	private static function save() {
		...
		$config = array(
			...
			'site_url'        => site_url(),
			'import_sub_type' => isset( self::$options['wpie_taxonomy_type'] ) ? (string) self::$options['wpie_taxonomy_type'] : '',
			'fields'          => self::$fields,
	...
	/**
	 * Retrieve ACF field metadata for config definition.
	 *
	 * @param string $field_id ACF field key/ID.
	 * @return array
	 */
	private static function get_acf_field_data( $field_id = '' ) {

		if ( empty( $field_id ) || ! function_exists( 'acf_get_field' ) ) {
			return array();
		}
		...
		$new_name = isset( $field['label'] ) && ! empty( $field['label'] ) ? strtolower( str_replace( ' ', '_', (string) preg_replace( '/[^a-z0-9_]/i', '', (string) $field['label'] ) ) ) : '';
	...
	/**
	 * Get list of standard WooCommerce product custom fields.
	 *
	 * @return array
	 */
	...
	/**
	 * Get list of standard user profile fields.
	 *
	 * @return array
	 */
	private static function get_user_fields() {

		return array( 'first_name', 'last_name', 'nickname', 'description' );
	}

	/**
	 * Get list of customer address fields.
	 *
	 * @return array
	 */
	...
	/**
	 * Get list of order custom fields.
	 *
	 * @return array
	 */
	...
	/**
	 * Process field options and populate mapped configuration fields.
	 *
	 * @return void
	 */
	private static function set_fields() {

		$fields = ( isset( self::$options['fields_data'] ) && trim( (string) self::$options['fields_data'] ) !== '' )
			? explode( '~||~', wpie_sanitize_field( wp_unslash( (string) self::$options['fields_data'] ) ) )
			: array();
		...
		if ( ! empty( $fields ) ) {
			...
			foreach ( $fields as $fieldData ) {
				...
				$field = explode( '|~|', (string) $fieldData );

				$label = isset( $field[0] ) ? wpie_sanitize_field( $field[0] ) : '';

				$options = isset( $field[1] ) ? json_decode( wpie_sanitize_field( $field[1] ), true ) : array();
				...
				$value = '{' . strtolower( (string) preg_replace( '/[^a-z0-9_]/i', '', $label ) ) . '[1]}';
				...
				if ( in_array( $value, $data, true ) ) {
					...
					while ( in_array( $tempval, $data, true ) ) {
						$tempval = '{' . strtolower( (string) preg_replace( '/[^a-z0-9_]/i', '', $label ) ) . '_' . $count . '[1]}';
						$count++;
					}
				...
				if ( 'wpie_cf' === $type ) {
					...
					$is_acf = isset( $options['is_acf'] ) ? absint( wpie_sanitize_field( $options['is_acf'] ) ) : 0;
					...
					if ( 'shop_order' === self::$export_type && in_array( $key, self::get_order_cf_fields(), true ) ) {
						continue;
					} elseif ( 'product' === self::$export_type && in_array( $key, self::get_product_cf_fields(), true ) ) {
					...
					} elseif ( ( in_array( self::$export_type, array( 'shop_customer', 'users' ), true ) && in_array( $key, self::get_user_fields(), true ) ) || ( 'shop_customer' === self::$export_type && in_array( $key, self::get_customer_fields(), true ) ) ) {
						self::$fields[ 'wpie_item_' . $key ] = $value;
					} else {

						$_uniqueid = uniqid();

						self::$fields['wpie_item_cf'][ $_uniqueid ] = array(
							'name'  => $key,
							'value' => $value,
						);
					}
				} elseif ( 'wpie-acf' === $type ) {

					$acf_key = isset( $options['acfKey'] ) && ! empty( $options['acfKey'] ) ? (string) $options['acfKey'] : '';

					if ( ! empty( $acf_key ) ) {

						$acf = array( $acf_key => self::get_acf_field_data( $acf_key ) );

						if ( ! empty( $acf ) ) {

							if ( ! isset( self::$fields['acf'] ) || ! is_array( self::$fields['acf'] ) ) {
								self::$fields['acf'] = array();
							}
					...
					continue;
				} elseif ( 'wpie_tax' === $type ) {

					$key = isset( $options['taxName'] ) ? wpie_sanitize_field( $options['taxName'] ) : '';

					if ( in_array( $key, array( 'product_type', 'product_shipping_class' ), true ) ) {

						self::$fields[ 'wpie_item_' . $key ] = $value;

						if ( 'product_shipping_class' === $key ) {
						...
						if ( isset( $options['hierarchical'] ) && 1 === (int) $options['hierarchical'] ) {
							self::$fields['wpie_item_taxonomy_hierarchical_delim'][ $key ] = '>';
						}
					}
				} elseif ( 'wc-product-attr' === $type ) {

					$attr_label = isset( $options['name'] ) ? (string) $options['name'] : '';

					$attr_name = ! empty( $attr_label ) ? strtolower( (string) preg_replace( '/[^a-z0-9_]/i', '', $attr_label ) ) : '';

					$temp_attr_name = '{attributename' . $attr_name . '[1]}';

					if ( isset( self::$fields['wpie_attr_slug'] ) && is_array( self::$fields['wpie_attr_slug'] ) && ! empty( self::$fields['wpie_attr_slug'] ) && in_array( $temp_attr_name, self::$fields['wpie_attr_slug'], true ) ) {

						$attr_count = 0;

						while ( in_array( $temp_attr_name, self::$fields['wpie_attr_slug'], true ) ) {
							$attr_count++;
					...
					if ( 'coupons' === $order_field_type && in_array( $order_field_key, array( '_cart_discount' ), true ) ) {
						continue;
					}
				} elseif ( 'yoast_seo' === $type ) {

					$key = isset( $options['metaKey'] ) ? wpie_sanitize_field( $options['metaKey'] ) : '';

					if ( in_array( $key, array( 'wpseo_twitter-image', 'wpseo_opengraph-image' ), true ) ) {
						self::$fields[ 'wpie_item_' . $key . '_url_data' ] = $value;
						$value                                             = 'url';
					}
					self::$fields[ 'wpie_item_' . $key ] = $value;
