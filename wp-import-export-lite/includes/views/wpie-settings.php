 ...
 * Settings view.
 *
 * Manages templates, access permissions, uninstall options, and background cron settings.
 *
...
$roles_obj  = function_exists( 'wp_roles' ) ? wp_roles() : ( isset( $GLOBALS['wp_roles'] ) ? $GLOBALS['wp_roles'] : null );
$user_roles = ( $roles_obj instanceof \WP_Roles ) ? $roles_obj->get_names() : array();
...
if ( file_exists( WPIE_CLASSES_DIR . '/class-wpie-common-action.php' ) ) {
	...
	if ( class_exists( 'WPIE_Common_Actions' ) ) {
		...
		$templates = method_exists( $cmm_act, 'wpie_get_templates' ) ? $cmm_act->wpie_get_templates() : array();
		unset( $cmm_act );
	}
}
...
if ( $wpie_bg_and_cron_processing && ! empty( $wpie_bg_and_cron_processing ) ) {
	...
	if ( is_array( $wpie_bg_and_cron_processing ) ) {
		$cron_method = isset( $wpie_bg_and_cron_processing['method'] ) ? $wpie_bg_and_cron_processing['method'] : 'wp';
...
$cron_url = add_query_arg( array( 'wpie_cron_token' => $token ), site_url( 'wp-load.php' ) );
?>
	...
	<div class="wpie_content_wrapper">
					...
					<div class="wpie_setting_element_data">
						<div class="wpie_setting_element">
							<form class="wpie_template_export" name="wpie_template_export" method="post" action="">
								...
								<select class="wpie_content_data_select wpie_template_list" name="wpie_template_list[]" multiple="multiple" data-placeholder="<?php esc_attr_e( 'Choose Templates', 'wp-import-export-lite' ); ?>">
									<?php
						...
						<form class="wpie_user_role_frm">
							...
							<div class="wpie_setting_element_only_data">
								...
								<div class="wpie_import_cap_wrapper">
									...
									<div class="wpie_import_cap_container">
										<input type="checkbox" class="wpie_checkbox wpie_update_superior_users wpie_user_cap" id="wpie_cap_update_superior_users" name="wpie_cap_update_superior_users" value="1"/>
										<label for="wpie_cap_update_superior_users" class="wpie_checkbox_label"><?php esc_html_e( 'Update Superior Users', 'wp-import-export-lite' ); ?></label>
									</div>
								</div>
							</div>
						</form>
		...
		<div class="wpie_section_wrapper">
			...
			<div class="wpie_section_content">
				<div class="wpie_setting_element_wrapper">
					<div class="wpie_import_cap_container">
						<input type="checkbox" class="wpie_checkbox wpie_delete_data_on_unistall" <?php checked( $delete_on_uninstall, 1 ); ?> id="wpie_delete_data_on_unistall" name="wpie_delete_data_on_unistall" value="1"/>
...
<?php
unset( $user_roles, $delete_on_uninstall, $templates, $cron_method, $token, $cron_url );
