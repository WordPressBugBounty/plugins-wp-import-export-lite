<?php
/**
 * New Import creation and configuration view.
 *
 * Provides the step-by-step import setup wizard, file options, filters, mapping, and execution modals.
 *
...
$wpie_import_type     = array();
$wpie_taxonomies_list = array();

if ( ! class_exists( '\wpie\import\WPIE_Import' ) && file_exists( WPIE_IMPORT_CLASSES_DIR . '/class-wpie-import.php' ) ) {
	require_once WPIE_IMPORT_CLASSES_DIR . '/class-wpie-import.php';
}

if ( class_exists( '\wpie\import\WPIE_Import' ) ) {
	$wpie_import          = new \wpie\import\WPIE_Import();
	$wpie_import_type     = method_exists( $wpie_import, 'wpie_get_import_type' ) ? $wpie_import->wpie_get_import_type() : array();
	$wpie_taxonomies_list = method_exists( $wpie_import, 'wpie_get_taxonomies' ) ? $wpie_import->wpie_get_taxonomies() : array();
	unset( $wpie_import );
}

$upload_sections = array();

if ( ! class_exists( '\wpie\import\upload\WPIE_Upload' ) && file_exists( WPIE_IMPORT_CLASSES_DIR . '/class-wpie-upload.php' ) ) {
	require_once WPIE_IMPORT_CLASSES_DIR . '/class-wpie-upload.php';
}

if ( class_exists( '\wpie\import\upload\WPIE_Upload' ) ) {
	$wpie_import_uploader = new \wpie\import\upload\WPIE_Upload();
	$upload_sections      = method_exists( $wpie_import_uploader, 'wpie_get_upload_section' ) ? $wpie_import_uploader->wpie_get_upload_section() : array();
...
if ( $import_id > 0 ) {
	$ref_id = Param::getSanitized( 'ref_id', 'text', '' );
...
$importUrl       = add_query_arg( array( 'page' => 'wpie-new-import' ), admin_url( 'admin.php' ) );
$manageImportUrl = add_query_arg( array( 'page' => 'wpie-manage-import' ), admin_url( 'admin.php' ) );
		...
		<form class="wpie_import_frm" method="post" action="#">
			...
			<div class="wpie_content_data">
					...
					<div class="wpie_section_wrapper wpie_default">
						...
						<div class="wpie_section_content wpie_show">
							<div class="wpie_upload_menu_wrapper">
								<?php
								if ( ! empty( $upload_sections ) && is_array( $upload_sections ) ) {
									$temp_flag = true;
								...
								<div class="wpie_import_type_outer_container">
									<div class="wpie_content_data_wrapper">
										<select class="wpie_content_data_select wpie_import_type_select" name="wpie_import_type">
											...
											<?php
											if ( ! empty( $wpie_import_type ) && is_array( $wpie_import_type ) ) {
												foreach ( $wpie_import_type as $key => $val ) {
													if ( in_array( $key, array( 'product', 'product_reviews', 'product_attributes', 'shop_order', 'shop_coupon', 'shop_customer' ), true ) ) {
														continue;
													}
													?>
											...
											<optgroup label="<?php esc_html_e( 'Premium Import Types', 'wp-import-export-lite' ); ?>" >
												...
												<option value="shop_customer"><?php esc_html_e( 'WooCommerce Customers', 'wp-import-export-lite' ); ?></option>
											</optgroup>
										</select>
									</div>
									<div class="wpie_content_data_wrapper wpie_taxonomies_types_wrapper">
										<select class="wpie_content_data_select wpie_taxonomies_types_select" name="wpie_taxonomy_type">
											<?php
											if ( ! empty( $wpie_taxonomies_list ) && is_array( $wpie_taxonomies_list ) ) {
								...
								<tr>
									...
									<td class="wpie_filter_data_label">
										<select class="wpie_content_data_select wpie_element_rule" name="">
											...
											<option value="equals"><?php esc_html_e( 'equals', 'wp-import-export-lite' ); ?></option>
											<option value="not_equals"><?php esc_html_e( 'not equals', 'wp-import-export-lite' ); ?></option>
											<option value="greater"><?php esc_html_e( 'greater than', 'wp-import-export-lite' ); ?></option>
											<option value="equals_or_greater"><?php esc_html_e( 'equals or greater than', 'wp-import-export-lite' ); ?></option>
											<option value="less"><?php esc_html_e( 'less than', 'wp-import-export-lite' ); ?></option>
											<option value="equals_or_less"><?php esc_html_e( 'equals or less than', 'wp-import-export-lite' ); ?></option>
											<option value="contains"><?php esc_html_e( 'contains', 'wp-import-export-lite' ); ?></option>
											<option value="not_contains"><?php esc_html_e( 'not contains', 'wp-import-export-lite' ); ?></option>
											<option value="is_empty"><?php esc_html_e( 'is empty', 'wp-import-export-lite' ); ?></option>
											<option value="is_not_empty"><?php esc_html_e( 'is not empty', 'wp-import-export-lite' ); ?></option>
										</select>
									</td>
									<td class="wpie_filter_data_label">
						...
						<div class="wpie_section_content wpie_show">
							<?php if ( current_user_can( 'wpie_add_shortcode' ) ) { ?>
								<div class="wpie_hint_wrapper">
									<div class="wpie_hint_text"><i class="far fa-question-circle wpie_hint_text_icon"></i><?php esc_html_e( 'Shortcode allowed in field. for more', 'wp-import-export-lite' ); ?> <a target="_blank" class="wpie_hint_link" href="<?php echo esc_url( 'https://plugins.vjinfotech.com/wordpress-import-export/documentation/add-shortcode/' ); ?>"><?php esc_html_e( 'click here', 'wp-import-export-lite' ); ?></a></div>
								</div>                                     
							<?php } else { ?>
								<div class="wpie_hint_wrapper">
									<div class="wpie_hint_text wpie_hint_text_warning"><i class="far fa-question-circle wpie_hint_text_icon"></i><?php esc_html_e( 'Please contact admin for allow shortcode in field. for more', 'wp-import-export-lite' ); ?> <a target="_blank" class="wpie_hint_link" href="<?php echo esc_url( 'https://plugins.vjinfotech.com/wordpress-import-export/documentation/add-shortcode/' ); ?>"><?php esc_html_e( 'click here', 'wp-import-export-lite' ); ?></a></div>
								</div>  
							<?php } ?>
				...
				<div class="wpie_section_container wpie_import_step5 wpie_import_step">
					...
					<?php
					if ( ! empty( $import_ext_html ) && is_array( $import_ext_html ) ) {
						foreach ( $import_ext_html as $_imp_html_file ) {
							if ( is_string( $_imp_html_file ) && file_exists( $_imp_html_file ) ) {
				...
				<div class="wpie_section_container wpie_import_step6 wpie_import_step">
					<div class="wpie_import_action_btn_wrapper wpie_import_action_top_btn_wrapper">
						<div class="wpie_import_action_container wpie_pre_import_btn">
							...
							<?php
							if ( ! empty( $final_btn_files ) && is_array( $final_btn_files ) ) {
								foreach ( $final_btn_files as $_btn_files ) {
									if ( is_string( $_btn_files ) && file_exists( $_btn_files ) ) {
										include $_btn_files;
									}
								}
							}
...
<?php
unset( $wpie_import_type, $wpie_taxonomies_list, $upload_sections, $final_btn_files, $import_ext_html, $importUrl, $manageImportUrl, $import_ref_id, $error_msg, $nonce, $import_id );
