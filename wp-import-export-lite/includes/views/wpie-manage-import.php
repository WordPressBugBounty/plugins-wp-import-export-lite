<?php
/**
 * Manage Import logs and templates view.
 *
 * Displays import history, statuses, records metrics, and reimport triggers.
 *
...
if ( file_exists( WPIE_CLASSES_DIR . '/class-wpie-common-action.php' ) ) {
	...
	if ( class_exists( 'WPIE_Common_Actions' ) ) {
		...
		$templates = method_exists( $cmm_act, 'get_import_list' ) ? $cmm_act->get_import_list() : array();
		unset( $cmm_act );
	}
}
...
<div class="wpie_main_container">
	...
	<div class="wpie_content_wrapper">
					...
					<tbody>
						...
						if ( ! empty( $templates ) && is_array( $templates ) ) {
							...
							foreach ( $templates as $template ) {
								...
								$importFile       = isset( $options['importFile'] ) && is_array( $options['importFile'] ) ? $options['importFile'] : array();
								$file             = isset( $importFile[ $activeFile ] ) && is_array( $importFile[ $activeFile ] ) ? $importFile[ $activeFile ] : array();
								...
								} elseif ( 'background' === $status || 'processing' === $status ) {
								...
								$uid            = uniqid();
								$last_update_ts = ! empty( $last_update_date ) ? strtotime( $last_update_date ) : false;
								$created_ts     = ! empty( $date ) ? strtotime( $date ) : false;
								$last_run_str   = ( false !== $last_update_ts && $last_update_ts > 0 ) ? date_i18n( $date_time_format, $last_update_ts ) : __( 'Never', 'wp-import-export-lite' );
								$created_str    = ( false !== $created_ts && $created_ts > 0 ) ? date_i18n( $date_time_format, $created_ts ) : '-';
								...
								<tr class="wpie_log_wrapper wpie_log_wrapper_<?php echo esc_attr( $id ); ?>">
									...
									<td class="wpie_log_data">
										<?php
										echo esc_html( __( 'Last run', 'wp-import-export-lite' ) . ' : ' . $last_run_str );
										?>
										<br />
										<?php
									...
									<td class="wpie_log_data"><?php echo esc_html( $created_str ); ?></td>
									...
									<td class="wpie_log_data wpie_action_<?php echo esc_attr( $status ); ?>">
										...
										<a class="wpie_log_action_btns wpie_process_reimport_btn" href="<?php echo esc_url( add_query_arg( array( 'page' => 'wpie-new-import', 'import_id' => $id, 'ref_id' => $uid, 'nonce' => wp_create_nonce( $id . $uid ) ), admin_url( 'admin.php' ) ) ); ?>"><i class="fas fa-redo-alt wpie_general_btn_icon wpie_data_tipso" data-tipso="<?php esc_attr_e( 'Reimport', 'wp-import-export-lite' ); ?>" aria-hidden="true"></i></a>
								...
								<?php
								unset( $date, $id, $opration_type, $last_update_date, $process_log, $options, $fileName, $status, $process_status, $last_update_ts, $created_ts, $last_run_str, $created_str, $uid );
							}
							unset( $date_format, $time_format, $date_time_format );
						} else {
							?>
							<tr>
								<td colspan="7">
		...
		<?php
		if ( ! empty( $ext_tab_files ) && is_array( $ext_tab_files ) ) {
...
<?php
unset( $templates, $ext_tab_files, $wpie_import_type, $wpie_taxonomies_list );
