<?php
/**
 * New Export creation and configuration view.
 *
 * Provides the step-by-step export setup wizard, filters, field editor, and process modals.
 *
...
if ( ! class_exists( '\wpie\export\WPIE_Export' ) && file_exists( WPIE_EXPORT_CLASSES_DIR . '/class-wpie-export.php' ) ) {
...
if ( class_exists( '\wpie\export\WPIE_Export' ) ) {
	$wpie_export          = new \wpie\export\WPIE_Export();
	$export_type          = method_exists( $wpie_export, 'get_export_type' ) ? $wpie_export->get_export_type() : array();
	$wpie_taxonomies_list = method_exists( $wpie_export, 'wpie_get_taxonomies' ) ? $wpie_export->wpie_get_taxonomies() : array();

	if ( class_exists( 'WooCommerce' ) && method_exists( $wpie_export, 'get_attribute_list' ) ) {
	...
	<div class="wpie_content_wrapper">
		<form class="wpie_export_frm" method="post" action="#">
					...
					<div class="wpie_section_content" style="display: block;">
						<div class="wpie_content_data_wrapper">
							<select class="wpie_content_data_select wpie_export_type_select" name="wpie_export_type">
								...
								<?php
								if ( ! empty( $export_type ) && is_array( $export_type ) ) {
						...
						<div class="wpie_content_data_wrapper wpie_taxonomies_types_wrapper wpie_sub_type_wrapper">
							<select class="wpie_content_data_select wpie_taxonomies_types_select" name="wpie_taxonomy_type">
								...
								<?php if ( ! empty( $wpie_taxonomies_list ) && is_array( $wpie_taxonomies_list ) ) { ?>                       
						...
						<div class="wpie_content_data_wrapper wpie_attribute_taxonomies_wrapper wpie_sub_type_wrapper">
							<select class="wpie_content_data_select wpie_attribute_taxonomies_select" data-placeholder="<?php esc_attr_e( 'All Attributes', 'wp-import-export-lite' ); ?>" name="wpie_attribute_taxonomy[]" multiple="multiple">
								<?php if ( ! empty( $attribute_taxonomies ) && is_array( $attribute_taxonomies ) ) { ?>                       
			...
			<div class="wpie_content_data">
					...
					<div class="wpie_section_content wpie_field_selection_wrapper">
						<div class="wpie_content_data_wrapper">
							...
							<div class="wpie_content_added_data_rule_wrapper">
								<table class="wpie_content_added_data_rule table table-bordered"></table>
							</div>
						</div>
					</div>
				</div>
						...
						<div class="wpie_content_data_wrapper">
							...
							<div class="wpie_fields_selection_btn_wrapper">
								...
								<div class="wpie_btn wpie_btn_secondary wpie_btn_radius wpie_add_bulk_fields">
									<i class="fas fa-plus wpie_general_btn_icon " aria-hidden="true"></i><?php esc_html_e( 'Add Bulk', 'wp-import-export-lite' ); ?>
								</div>
								<div class="wpie_btn wpie_btn_secondary wpie_btn_radius wpie_fields_add_all">
									<i class="fas fa-plus wpie_general_btn_icon " aria-hidden="true"></i><?php esc_html_e( 'Add All', 'wp-import-export-lite' ); ?>
								</div>
					...
					<div class="wpie_section_content">
						<div class="wpie_content_data_wrapper">
							<table class="wpie_content_data_tbl table table-bordered">
								...
								<?php
								if ( ! empty( $advance_options_files ) && is_array( $advance_options_files ) ) {
									$temp = 0;
									foreach ( $advance_options_files as $adv_options ) {
										...
										if ( is_string( $adv_options ) && file_exists( $adv_options ) ) {
											include $adv_options;
										}
										if ( $temp % 2 === 1 ) {
											?>
				...
				<?php
				if ( ! empty( $extension_html_files ) && is_array( $extension_html_files ) ) {
					foreach ( $extension_html_files as $ext_html_file ) {
						if ( is_string( $ext_html_file ) && file_exists( $ext_html_file ) ) {
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="wpie_process_bar_inner_wrapper">
					...
					<?php if ( ! empty( $wpie_remote_data ) && is_array( $wpie_remote_data ) ) { ?>
						<div class="wpie_remote_export_wrapper">
							...
							<table class="wpie_remote_export_table table table-bordered">
								<?php foreach ( $wpie_remote_data as $remote_key => $remote_data ) { ?>
									<tr>
										<td>
											<input type="checkbox" class="wpie_checkbox" id="wpie_remote_export_<?php echo esc_attr( $remote_key ); ?>" name="wpie_remote_exported_data[]" value="<?php echo esc_attr( $remote_key ); ?>"/>
											<label for="wpie_remote_export_<?php echo esc_attr( $remote_key ); ?>" class="wpie_checkbox_label"><?php echo isset( $remote_data['label'] ) ? esc_html( $remote_data['label'] ) : ''; ?></label>
										</td>
										<td>
											<div class="wpie_content_data_wrapper">
												<select class="wpie_content_data_select" name="wpie_export_type" multiple="multiple">
													<?php $remote_options = isset( $remote_data['data'] ) && is_array( $remote_data['data'] ) ? $remote_data['data'] : array(); ?>
				...
				<div class="modal-footer">
					<div class="wpie_export_process_option_btn_wrapper">
						...
						<?php
						if ( ! empty( $extension_process_btn ) && is_array( $extension_process_btn ) ) {
							foreach ( $extension_process_btn as $ext_p_btn ) {
								if ( is_string( $ext_p_btn ) && file_exists( $ext_p_btn ) ) {
									include $ext_p_btn;
								}
							}
						}
					...
					<div class="wpie_export_process_btn_wrapper ">
						...
						<a class="wpie_btn wpie_btn_primary wpie_export_process_btn wpie_export_manage_export_btn" href="<?php echo esc_url( add_query_arg( array( 'page' => 'wpie-manage-export' ), admin_url( 'admin.php' ) ) ); ?>">
...
<?php
unset( $export_type, $wpie_taxonomies_list, $attribute_taxonomies, $advance_options_files, $extension_html_files, $extension_process_btn, $wpie_remote_data );
