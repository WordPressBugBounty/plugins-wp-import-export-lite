<?php
/**
 * Manage Export logs and templates view.
 *
 * Displays export history, statuses, and action triggers.
 *
...
if ( file_exists( WPIE_CLASSES_DIR . '/class-wpie-common-action.php' ) ) {
	...
	if ( class_exists( 'WPIE_Common_Actions' ) ) {
		...
		$templates = method_exists( $cmm_act, 'get_export_list' ) ? $cmm_act->get_export_list() : array();
		unset( $cmm_act );
	}
}
					...
					<tbody>
						...
						if ( ! empty( $templates ) && is_array( $templates ) ) {
							...
							foreach ( $templates as $template ) {
								...
								<?php
								unset( $date, $id, $opration_type, $last_update_date, $process_log, $options, $fileName, $status, $process_status, $last_update_ts, $created_ts, $last_run_str, $created_str );
							}
...
<?php
unset( $templates, $export_type, $wpie_taxonomies_list, $ext_tab_files );
